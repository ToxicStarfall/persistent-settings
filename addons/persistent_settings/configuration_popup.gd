...
var plugin
#var plugin.settings: ConfigFile
...
func _connect_buttons():
	...
	_initialize_file_viewer_screen()
...
func _initialize_welcome_screen():
	...
	WelcomeScreen.get_node("%ImportSelectedButton").pressed.connect( func(): #.bind())
			var arr = []
			for import_option in plugin.settings.get_value("General", "import_options", {}).keys():
				if plugin.settings.get_value("General", "import_options", {})[import_option] == true:
	...
	WelcomeScreen.get_node("%SaveSelectedButton").pressed.connect( func(): #.bind())
			var arr = []
			for save_option in plugin.settings.get_value("General", "save_options", {}).keys():
				if plugin.settings.get_value("General", "save_options", {})[save_option] == true:
	...
	var import_options: Dictionary = plugin.settings.get_value("General", "import_options", {})
	for i in import_options.keys():
		if ImportOptions.has_node(NodePath(i)):# + "/CheckBox")):
			...
			group.get_node("CheckBox").pressed.connect( func():
				...
				plugin.settings.set_value("General", "import_options", import_options))
			group.get_node("Button").pressed.connect( file_import_requested.emit
				.bind( FileConstants.get(group.name) ))

	var save_options: Dictionary = plugin.settings.get_value("General", "save_options", {})
	for i in save_options.keys():
		if SaveOptions.has_node(NodePath(i)):# + "/CheckBox")):
			...
			group.get_node("CheckBox").pressed.connect( func():
				...
				plugin.settings.set_value("General", "save_options", save_options))
	...
	WelcomeScreen.get_node("%PresetOptions/%DeletePresetButton").pressed.connect( func():
			...
			if !preset_idx == 0:
				...
				apply_plugin_settings(plugin.settings)
	...
	WelcomeScreen.get_node("%PresetOptions/%SavePresetButton").pressed.connect( func(): #.bind())
			...
			if regex.search(preset_name) != null:
				# Invalidate preset names with non file name safe characters.
				push_warning("[persistent_settings] Invalid preset name. Please only include alphanumeric characters and or - _ +")
				return
			var regex_output = regex.sub(preset_name, "-", true)
			preset_name = regex_output
...
func _initialize_general_screen():
	...
	GeneralSettingsGroup.get_node("show_on_launch").pressed.connect( func():
		plugin.settings.set_value("General", "show_on_launch", GeneralSettingsGroup.get_node("show_on_launch").button_pressed))

	#file_name = "project.godot"
	ProjectSettingsGroup.get_node("include_metadata").pressed.connect( func():
		plugin.settings.set_value("ProjectSettings", "include_metadata", ProjectSettingsGroup.get_node("include_metadata").button_pressed))
...
func apply_plugin_settings(settings: ConfigFile):
	...
	for import_option in plugin.settings.get_value("General", "import_options", {}).keys():
		if ImportOptions.has_node(import_option + "/CheckBox"):
			ImportOptions.get_node(import_option + "/CheckBox").button_pressed = settings.get_value("General", "import_options", {})[import_option]
	for save_options in plugin.settings.get_value("General", "save_options", {}).keys():
		if SaveOptions.has_node(save_options + "/CheckBox"):
			SaveOptions.get_node(save_options + "/CheckBox").button_pressed = settings.get_value("General", "save_options", {})[save_options]
	...
	GeneralSettingsGroup.get_node("show_on_launch").button_pressed = settings.get_value("General", "show_on_launch", false)

	var ProjectSettingsGroup = %BasicScreen/%ProjectSettingsSection
	ProjectSettingsGroup.get_node("include_metadata").button_pressed = settings.get_value("ProjectSettings", "include_metadata", false)
...
func populate_dropdown(dropdown: OptionButton, dropdown_values: Array, start_idx: int = 0):
	...
	for value in dropdown_values:  dropdown.add_item(value)
